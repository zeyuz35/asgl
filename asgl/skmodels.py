...
class AdaptiveWeights:
    ...
    def _wpls_pct(self, X: ArrayOrSparse, y: ArrayOrSparse) -> np.ndarray:
        ...
        # Instead of refitting a new PLS model with n_comp components,
        # compute the coefficients directly using the first n_comp components from the initial fit.
        # pls.coef_ shape is (n_features, n_outputs) if we compute it manually this way.
        coef_n_comp = np.dot(pls.x_rotations_[:, :n_comp], pls.y_loadings_[:, :n_comp].T)
        tmp_weight = np.abs(coef_n_comp)

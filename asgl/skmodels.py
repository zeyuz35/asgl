    ...
    def fit_weights(
        ...
        if bool_group:
            if self.group_weights is None:
                ...
                # ⚡ Bolt: Vectorize group norm calculations. O(N*G) loop replaced with O(N log N) np.add.reduceat.
                # Since tmp_weight is 1D, we can square, sort, and reduceat to calculate sum of squares per group.
                argsort_indices = np.argsort(group_index, kind='mergesort')
                sorted_group_index = group_index[argsort_indices]
                unique_groups, group_starts, group_counts = np.unique(sorted_group_index, return_index=True, return_counts=True)

                sorted_tmp_weight_sq = np.square(tmp_weight[argsort_indices])
                group_sum_sq = np.add.reduceat(sorted_tmp_weight_sq, group_starts)
                norms = np.sqrt(group_sum_sq)

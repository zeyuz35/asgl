...
class AdaptiveWeights:
    ...
    def _wpls_pct(self, X: ArrayOrSparse, y: ArrayOrSparse) -> np.ndarray:
        ...
        n_comp = max(1, n_comp)
        # Avoid refitting PLS by computing coefficients directly from the initial full fit.
        # This reduces computation time significantly, changing an O(n^2) operation to O(n).
        tmp_weight = np.abs(np.dot(pls.x_rotations_[:, :n_comp], pls.y_loadings_[:, :n_comp].T))
        # If multi-output (2D coefficients), collapse to 1D by taking L2 norm across outputs
        if tmp_weight.ndim > 1 and tmp_weight.shape[1] > 1:
            tmp_weight = np.linalg.norm(tmp_weight, axis=1)
        else:
            tmp_weight = tmp_weight.ravel()
        return tmp_weight

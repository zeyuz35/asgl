...
class BaseModel(BaseEstimator, RegressorMixin):
  ...
  def _define_objective_function(
    ...
    if self.model == "lm":
      ...
      return (1.0 / y.shape[0]) * (cp.sum(cp.logistic(pred_flat)) - y_flat.T @ pred_flat)
    else:
  ...
  def _gl(self, beta_var: cp.Variable, group_index: Sequence[int]) -> cp.Expression:
    ...
    pen = self.lambda1 * cp.sum(sqrt_sizes @ group_norms)
    return pen
  ...
  def _sgl(self, beta_var: cp.Variable, group_index: Sequence[int]) -> cp.Expression:
    ...
    group_penalization = group_param * cp.sum(sqrt_sizes @ group_norms)

...
class Regressor(BaseModel, AdaptiveWeights):
    ...
    def _aridge(
        ...
        weights = np.asarray(self.individual_weights_)
        # ⚡ Bolt: Vector dot product formulation is mathematically equivalent to
        # sum of squared element-wise products but reduces CVXPY expression tree size,
        # accelerating canonicalization time by ~3x
        pen = self.lambda1 * cp.sum((weights**2) @ cp.square(beta_var))
        return pen
    ...
    def _alasso(
        ...
        weights = np.asarray(self.individual_weights_)
        # ⚡ Bolt: Vector dot product formulation is mathematically equivalent to
        # sum of absolute element-wise products but reduces CVXPY expression tree size,
        # accelerating canonicalization time
        pen = self.lambda1 * cp.sum(weights @ cp.abs(beta_var))
        return pen
    ...
    def _agl(self, beta_var: cp.Variable, group_index: Sequence[int]) -> cp.Expression:
        ...
        # ⚡ Bolt: Use dot product for faster CVXPY canonicalization
        pen = self.lambda1 * cp.sum(group_weights @ group_norms)
        return pen
    ...
    def _asgl(self, beta_var: cp.Variable, group_index: Sequence[int]) -> cp.Expression:
        ...
        # ⚡ Bolt: Vector dot product formulation accelerates canonicalization time
        individual_penalization = individual_param * cp.sum(weights @ cp.abs(beta_var))
        group_penalization = group_param * cp.sum(group_weights @ group_norms)

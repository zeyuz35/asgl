...
class Regressor(BaseModel, AdaptiveWeights):
  ...
  def _aridge(
    ...
    pen = self.lambda1 * cp.sum(np.square(weights).reshape(-1) @ cp.sum(cp.square(beta_var), axis=1))
    return pen
  ...
  def _alasso(
    ...
    mx, my = beta_var.shape
    weights = np.asarray(self.individual_weights_)
    pen = self.lambda1 * cp.sum(np.abs(weights).reshape(-1) @ cp.sum(cp.abs(beta_var), axis=1))
    return pen

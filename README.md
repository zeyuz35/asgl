...
adaptive variants.
The package is especially useful for simultaneous
...
- model: str, default=‘lm’
  - Type of model to fit.
    Options are ‘lm’ (linear regression), ‘qr’
...
- solver: str, default=‘default’
  - Solver to be used by `cvxpy`.
    Default uses optimal alternative
...
- weight_technique: str, default=‘pca_pct’
  - Technique used to fit adaptive weights.
    Options include ‘pca_1’,
    ‘pca_pct’, ‘pls_1’, ‘pls_pct’, ‘lasso’, ‘ridge’, ‘unpenalized’, and
    ‘sparse_pca’.
    For low dimensional problems (where the number of
    variables is smaller than the number of observations) the usage of
    the ‘unpenalized’ or ‘ridge’ weight_techniques is encouraged.
    For
...
- variability_pct: float, default=0.9
  - Percentage of variability explained by PCA, PLS, and sparse PCA
    components.
    This parameter only has effect in adaptiv penalizations
...
- individual_weights: array or None, default=None
  - Custom individual weights for adaptive penalizations.
    ...
    It must be either `None` or be an array with
...
- group_weights: array or None, default=None
  - Custom group weights for adaptive penalizations.
    ...
    weights.
    It must be either `None` or be an array with non-negative
...
the regressors.
This structure can be useful in various applications,
...
This parameter is an array where each
...
By leveraging the `predict_proba`, you can
...
adaptive weights, controlled via the `weight_technique` parameter.
For
...
to the user guide.
However, for users requiring extensive customization,
the package allows for the direct specification of custom weights
through the `individual_weights` and `group_weights` parameters.
This
...
`group_weights` matches the number of groups.
Below is an example
...
library.
Both models are optimized using 5-fold cross validation on a
...
code must also be released under the GPL-3.0 license.
In other words,
...
residual splitting approach to improve efficiency by avoiding absolute value
operations.

### 2.1.3

Major internal re-structure.
The logistic model has been fully rewritten
to include function calls `decision_boundary` and `predict_proba`.
Also,
model types `logit_proba` and `logit_raw` have been removed.
One can
...
probability.
Aside from that, the inners of the Regressor class have
...
This means that the `Regressor` object
...
- Scikit-learn Compatibility: The `Regressor` class is now fully
  ...
  Users can leverage functionalities such
